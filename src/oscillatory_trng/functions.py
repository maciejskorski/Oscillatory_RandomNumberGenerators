...
def brownian_mod1_logp(mu,sigma,t,y):
    ...
    """
    # align dimensions and pad zeros at start
    pad_dim = [(1,0)]+(len(y.shape)-1)*[(0,0)]
    y = np.pad(y,pad_dim,mode='constant')
    t = np.expand_dims(t,axis=list(np.arange(1,len(y.shape))))
    t = np.pad(t,pad_dim,mode='constant')
